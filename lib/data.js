...
lib.deleteCallback = (customDirectory, fileName, callback) => {
  const pathToOpenFile = `${lib.baseUrl}${customDirectory}/${fileName}.json`;
  fs.unlink(pathToOpenFile, (err) => {
    if (!err) {
      callback(false);
    } else {
...
lib.deleteAsync = async (customDirectory, fileName, callback) => {
  try {
    const pathToOpenFile = `${lib.baseUrl}${customDirectory}/${fileName}.json`;
    await fs.promises.unlink(pathToOpenFile);
    callback(false);
  } catch (error) {
    callback("Error deleting file", error.message);
  }
};
